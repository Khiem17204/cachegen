...
class Quantizer:
    ...
    def quantize(tensor: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Args:
            tensor: Arbitrary-shape floating-point tensor (typically FP16).

        Returns:
            A ``(quantized, scales)`` tuple where:

            - **quantized** — INT8 tensor with the same shape as *tensor*.
        ...
        quantized = torch.clamp(torch.round(tensor / scales), min=-128, max=127).to(
            torch.int8
        )
